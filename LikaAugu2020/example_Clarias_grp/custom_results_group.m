...
% created by Bas Kooijman 2019/03/11
  ...
  % text to be written in the figure to identify estimation conditions
  parFlds = fieldnames(parGrp); n_parFlds = length(parFlds); shareTxt = 'share: '; diffTxt = 'diff: '; varTxt = '';
  for i = 1:n_parFlds
  ...
  if isfield(metaPar, 'weights')
    varTxt = 'var-reduc: '; parFlds = fieldnames(metaPar.weights); n_parFlds = length(parFlds);
    for i = 1:n_parFlds
       if ~metaPar.weights.(parFlds{i}) == 0
         varTxt = [varTxt, parFlds{i}, ', '];
       end
    end
  end
  ...
  text(10,800, varTxt, 'Interpreter', 'none')
