function JusuKooy2025_SI(fig)
% Supporting Information for JusuKooy2025
...
if ~exist('fig','var')
 ...
 legend = { ... % 
   ...
   {'o', 4, 2, [0 0 1], [0 0 1]}, 'Squamata';
   {'o', 4, 2, [1 0 1], [1 0 1]}, 'Aves';
 ...
 switch fig(i)
   ...
   case 4
     ...
     vars = read_allStat({'E_0','kap_R','L_b','E_m','mu_V','M_V','s_Hbp'});
     E_0=vars(:,1); kap_R=vars(:,2); L_b=vars(:,3); E_m=vars(:,4); mu_V=vars(:,5); M_V=vars(:,6);s_Hbp=vars(:,7);
     %kap_R = 0.95 + 0*kap_R; % remove simultaneous hermaphrodite allocation
     
     effR = kap_R.*L_b.^3.*(M_V.*mu_V+E_m)./E_0; % total reproduction efficiency
     [hsHbp_effR hleg_sHbp_effR] = shstat([s_Hbp,effR], legend, 'Mollusca & Squamata & Aves & Placentalia'); 
     figure(hsHbp_effR)
     xlabel('_{10}log precociality index, _{10}log s_H^{bp}, -')
     ylabel('total reproduction efficiency, -')
     %print -r0 -dpng sHbp_effR.png
     
     figure(hleg_sHbp_effR)
     %print -r0 -dpng legend_sHbp_effR.png
   ...
   case 5
     ...
     effR = kap_R.*L_b.^3.*(M_V.*mu_V+E_m)./E_0; % total reproduction efficiency
     [hss_effR hleg_ss_effR] = shstat([s_s,effR], legend, 'Mollusca & Squamata & Aves & Placentalia'); 
     figure(hss_effR)
