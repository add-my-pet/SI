...
if k_relax < 0 || k_relax > 2 || k_hurry < 0; info = 0; prdData = []; return; end
if lambda_relax < 0 || lambda_hurry < 0; info = 0; prdData = []; return; end
if p_Mshape < 0 || k < 0; info = 0; prdData = []; return; end

p_M = 10.^surv_pM(:,1); % undo log10-transform
p_relax = 2./(1+exp((p_M/p_Mshape).^k)); % prob on relax spec
